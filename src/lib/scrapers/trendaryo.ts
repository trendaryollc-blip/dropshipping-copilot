...
function assertServerSide() {
  if (typeof window !== 'undefined') {
    throw new Error(
      'TrendaryoScraper can only be used server-side. Call via /api/trendaryo/* endpoints.'
    );
  }
}
...
export class TrendaryoScraper {
  ...
  // Safety check: JSDOM only works server-side
  private static assertServerSide() {
    if (typeof window !== 'undefined') {
      throw new Error(
        'TrendaryoScraper can only be used server-side. Call via /api/trendaryo/* endpoints.'
      );
...
let _instance: TrendaryoScraper | null = null;
export function trendaryoScraper(): TrendaryoScraper {
  if (!_instance) {
    assertServerSide();
    _instance = new TrendaryoScraper();
  }
  return _instance;
}
