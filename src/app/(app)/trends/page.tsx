      ...
      <section className="relative overflow-hidden rounded-3xl border border-border/50 bg-card/60 p-6 backdrop-blur-sm sm:p-8 animate-in">
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="space-y-2">
            ...
            <h1 className="hero-title">Market Trends</h1>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-card/60 p-5 backdrop-blur-sm animate-in delay-1">
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-foreground">Search Trends</h3>
          </div>
          <Input placeholder={'e.g. "wireless earbuds" or "summer fashion"'} className="h-9" />
          <Button className="w-full rounded-xl">Search</Button>

          <AIActionButton
            task="seo_optimization"
            input={{ productName: "Trending product", niche: "Dropshipping", targetKeywords: ["trending products", "seasonal demand", "market trends"] }}
            label="AI Trend Insights"
            onSuccess={() => {
              toast.success("AI trend insights ready")
            }}
          />
        </div>
      </div>

      {/* Placeholder for trend cards */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="relative overflow-hidden rounded-xl border border-border/50 bg-card/60 p-4 backdrop-blur-sm animate-in delay-1">
            ...
            <h4 className="font-semibold text-foreground mb-2">Trend #{i}</h4>
            <p className="text-xs text-muted-foreground">Placeholder for trend data and charts.</p>
